...
function writeLog(log){
  try{
    fs.appendFileSync('/home/mqm/debug-loopback.log',log +'\r\n')
  }catch(e){
...
MongoDB.prototype.parseObjToDatabase = function(model, data){
  var updateOperator = ['$set', '$push', '$pull', '$setOnInsert', '$pop', '$pullAll', '$addToSet', '$each'];
  var self = this;
  updateOperator.forEach(function(field){
    if(data[field]){
      data[field] = self.toDatabase(model, data[field]);
    }
  });
...
MongoDB.prototype.execute = function(model, command) {
   ...
   writeLog(`execute: () ${command} ${JSON.stringify(args)}`);
...
MongoDB.prototype.create = function(model, data, options, callback) {
  ...
  writeLog('create 0 ' + JSON.stringify(data));

  convertDataToMongoObject(data, this._models[model].properties);
  data = self.toDatabase(model, data);
  writeLog('create ' + JSON.stringify(data));
...
MongoDB.prototype.buildWhere = function(model, where, options) {
  ...
  // parse where by JSON Stringify
  if (where.whereStr) {
    var condition = JSON.parse(where.whereStr);
    Object.assign(where, condition);
    delete where.whereStr;
  }
  convertDataToMongoObject(where, self._models[model].properties);
...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  if(filter.where && filter.where.customMethod === 'customFind'){
    delete filter.where.customMethod;
    return self.customFind(model, filter, options, callback);
  }
 ...
 * @param {Function} [callback] The callback function
 */
MongoDB.prototype.customFind = function customFind(model, filter, options, callback) {
  var self = this;
  if (self.debug) {
    debug('all', model, filter);
  }
...
MongoDB.prototype.update = MongoDB.prototype.updateAll = function updateAll(
  ...
  var self = this;
  if(where.customMethod){
    var customMethod = where.customMethod;
    delete where.customMethod;
    if (!this[customMethod]){
      return cb(new Error('Invalid method ' + customMethod));
    }
    return this[customMethod](model,
      where,
      data,
      options,
      cb);
  ...
  writeLog('updateAll where ' + JSON.stringify(where));
  writeLog('updateAll data ' + JSON.stringify(data));
  ...
  delete data[idName];
  data = self.parseObjToDatabase(model, data);
  ...
  writeLog('updateAll parseUpdateData ' + JSON.stringify(data));
...
function isMongoIDString(value){
  return typeof value === 'string' && /^[0-9a-fA-F]{24}$/.test(value);
}

function getTypeOfField(props, field) {
  return props[field].type.name;
}

function isDateType(props, field) {
  return props && props[field] && props[field].type.name === 'Date';
}

function isDefinedProperty(props, field) {
  return props && props[field];
}

function convertDefinedDataToMongoObject(data, type) {
  Object.keys(data).forEach(function (key) {
    if (typeof data[key] === 'object') {
      convertDefinedDataToMongoObject(data[key], type);
    } else {
      if (isMongoIDString(data[key])) {
        writeLog('isMongoIDString: ' + data[key]);
        data[key] = ObjectID(data[key]);
      } else if (type === 'Date') {
        writeLog('isDATE: ' + data[key]);
...
function convertDataToMongoObject(data, props) {
  Object.keys(data).forEach(function(key){
    if (data[key]){
      if (typeof data[key] === 'object') {
        if (isDefinedProperty(props, key)) {
          convertDefinedDataToMongoObject(data[key], getTypeOfField(props, key));
        } else {
          convertDataToMongoObject(data[key], props);
        }
      } else {
        if (isMongoIDString(data[key])) {
          writeLog('isMongoIDString2: ' + data[key]);
          data[key] = ObjectID(data[key]);
        } else if (isDateType(props, key)) {
          writeLog('isDATE2: ' + data[key]);
          data[key] = new Date(data[key]);
        }
      }
    }   
...
/**
 * CustomUpdate one matching instances
 * @param {String} model The model name
 * @param {Object} where The search criteria
 * @param {Object} data The property/value pairs to be updated
 * @callback {Function} cb Callback function
 */
MongoDB.prototype.customUpdateOne = function customUpdateOne(
  model,
  where,
  data,
  options,
  ...
  var self = this;
  writeLog('updateOne where ' + JSON.stringify(where));
  writeLog('updateOne data ' + JSON.stringify(data));
  convertDataToMongoObject(data, this._models[model].properties);

  if (self.debug) {
    debug('updateOne', model, where, data);
  }
  var idName = this.idName(model);

  where = this.buildWhere(model, where, options);

  delete data[idName];

  data = this.parseObjToDatabase(model, data);

  // Check for other operators and sanitize the data obj
  data = this.parseUpdateData(model, data, options);

  writeLog('updateOne parseUpdateData ' + JSON.stringify(data));
  ...
  this.execute(
    ...
    function (err, info) {
      ...
      if (self.debug)
        debug('updateOne.callback', model, where, data, err, info);
...
/**
 * CustomFindAndUpdateOne one matching instances
 * @param {String} model The model name
 * @param {Object} where The search criteria
 * @param {Object} data The property/value pairs to be updated
 * @callback {Function} cb Callback function
 */
MongoDB.prototype.customFindAndUpdateOne = function customFindAndUpdateOne(
  model,
  where,
  data,
  options,
  ...
  var self = this;
  writeLog('customFindAndUpdateOne where ' + JSON.stringify(where));
  writeLog('customFindAndUpdateOne data ' + JSON.stringify(data));
  ...
  writeLog('customFindAndUpdateOne parseUpdateData ' + JSON.stringify(data));
  findAndUpdateOne(self, model, where, data, cb);
};

function findAndUpdateOne(self, model, where, data, cb) {
  self.execute(
    model,
    'findOneAndUpdate',
    where,
    ...
    function (err, result) {
      ...
      writeLog('err:: ' + err +JSON.stringify(err));
      var idName = self.idName(model);
      var object = result && result.value;
      if (!err && !object) {
        // No result
      ...
      self.setIdValue(model, object, object._id);
      if (object && idName !== '_id') {
        delete object._id;
      }
      if (cb) {
