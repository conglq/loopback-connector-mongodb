...
MongoDB.prototype.execute = function(model, command) {
  ...
  writeLog('execute  ' + (new Error).stack)
  writeLog('execute]]]]] '+ model.toString() +' : '+ command.toString() + ' [] ' + JSON.stringify(args))
...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  writeLog('MongoDB.prototype.all  ' + (new Error).stack)
  var self = this;
  if (self.debug) {
    debug('all', model, filter);
  }

  if(filter.where.customMethod === 'customFind'){
...
MongoDB.prototype.customFind = function customFind(model, filter, options, callback) {
  writeLog('MongoDB.prototype.customFind  ' + (new Error).stack)
  var self = this;
  if (self.debug) {
    debug('all', model, filter);
  }
  ...
  if (filter.where) {
    if(filter.where.whereStr) {
      writeLog('customFind')
      var condition = JSON.parse(filter.where.whereStr)
      convertDataToMongoObject(condition);
      Object.assign(filter.where,condition );
      delete filter.where.whereStr;
    }
    query = self.buildWhere(model, filter.where, options);
  }
  var fields = filter.fields;
...
MongoDB.prototype.update = MongoDB.prototype.updateAll = function updateAll(
  ...
  var self = this;
  if(where.customMethod === 'customUpdateOne'){
    delete where.customMethod;
    return this.customUpdateOne( model,
      where,
      data,
      options,
      cb)
...
function convertDataToMongoObject(data){
  writeLog('convertDataToMongoObject typeof'+  data);
  Object.keys(data).forEach(function(key){
    if(typeof data[key] === 'object'){
      convertDataToMongoObject(data[key]);
    }else {
...
MongoDB.prototype.customUpdateOne = function customUpdateOne(
  ...
  writeLog('updateOne data ' + JSON.stringify(data) + data);
  convertDataToMongoObject(data);
  if (self.debug) {
...
MongoDB.prototype.isObjectIDProperty = function(model, prop, value, options) {
  if (
    ...
    writeLog('isObjectIDProperty: '+ /^[0-9a-fA-F]{24}$/.test(value) +' model '+ model + ' prop ' + prop + ' value '+value +' options '+ JSON.stringify(options));
    return true;
